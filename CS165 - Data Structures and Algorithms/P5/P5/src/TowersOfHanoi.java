...
public class TowersOfHanoi {
    ...
    public static void iterativeHanoi(int disc, int from, int aux, int to) {
        // your code here
        Move move = new Move(disc, from, aux, to, true);
        moves.push(move);
        while(!moves.isEmpty()){
            Move m = moves.pop();
            if (m.disc > 0) {
                if (m.isCovered) {
                    m.isCovered = false;
                    moves.push(m);
                    Move newMove = new Move(disc - 1, from, to, aux, false);
                    moves.push(newMove);
                }
                else {
                    move(m.disc, m.from, m.to);
                    Move newMove = new Move(disc - 1, aux, from, to, false);
                    moves.push(newMove);
