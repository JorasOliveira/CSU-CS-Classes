...
public abstract class Animal {
    ...
    double computeDistance(int row0, int col0, int row1, int col1) {
        double first = row0 - row1; // computes(x - a)
        double second = col0 - col1; // computes (y - b)
        double firstSqr = Math.pow(first, 2.0); // squares (x - a)
        double secondSqr = Math.pow(second, 2.0); // squares (y - b)
        double sqrt = Math.sqrt((firstSqr + secondSqr)); // computes the sqrt of ( (x - a)^2 + (y - b)^2 )
        return sqrt;
    }
