...
public class Terrier extends Animal {
    ...
    public eMove findMove() {
        // STUDENT CODE HERE
        boolean up = currentRow > closestRow;
        boolean down = currentRow < closestRow;
        boolean right = currentCol < closestCol;
        boolean left = currentCol > closestCol;
    ...
    public boolean isValid(int row, int col){
        ...
        if (stayOnBoard(row, col)){
            if (field[row][col] == 'D' || field[row][col] == 'T' || field[row][col] == 'F'){
                return false;
            }
            return true;
        }
